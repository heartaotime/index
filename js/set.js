...
layui.use('form', function () {
    ...
    form.on('submit(submit-del)', function (data) {
        if (!userInfo) {
            layer.msg("请先登陆");
            return;
        }
        ...
        layer.confirm('确认删除[' + field.menuname + ']吗?', {icon: 3, title: '提示'}, function (index) {
            layer.close(index);
            ...
            Util.postJson("./common-server/user/api/v1/delIndex", param, function (response) {
                ...
                refreshSelect(function () {
                    layer.msg('删除[' + field.menuname + ']成功，返回到导航页刷新页面即可看到效果哦');
                    form.val("index-edit", {
                        "index": -1,
                        "menuname": "",
                        "menuurl": "",
    ...
    Util.postJson("./common-server/user/api/v1/getConfig", param, function (response) {
        ...
        if (result.length > 0) {
            ...
            if (config.weatherSwitch) {
                res.weatherswitch = 'on';
            }
            if (config.searchInputShow) {
                res.searchinputshow = 'on';
            }
            if (config.backgroundImgShow) {
                res.backgroundimgshow = 'on';
            }
